...
Invoice.prototype = {
	...
	calcTotal: function () {
		jQuery($.opt.parentClass).each(function (i) {
			var row = jQuery(this);
			var discount = row.find($.opt.price).val() * row.find($.opt.qty).val();
			var total = discount - row.find($.opt.line_discount).val();
	...
	change: function () {
		jQuery($.opt.parentClass).each(function (i) {
			...
			var discount = row.find($.opt.item).val();
			row.find($.opt.price).val(discount);
	...
	newRow: function () {
		...
		jQuery(".item-row:last").after('<div class="form-row item-row"><div class="form-group col-md-4"><label class="onboard" for="item">Item*</label><select class="form-control form-control-sm item" id="item"><option selected disabled>Choose Value</option><option value="50">Value 50</option><option value="70">Value 70</option></select></div><div class="form-group col-md-4"><label class="onboard" for="description">Description</label><input type="text" class="form-control dark form-control-sm" id="description" placeholder=""></div><div class="form-group col-md-3"><label class="onboard" for="unit_price">Unit Price</label><input type="text" class="form-control dark form-control-sm price" id="price" placeholder=""></div><div class="delete-btn form-group col-md-1 text-center"><a ><img src="img/custom-icons/delete.svg" alt="delete" class=' + $.opt.delete.substring(1) +' /></a></div><div class="form-group col-md-2"><label class="onboard" for="quantity">Quantity</label><input type="text" class="form-control dark form-control-sm qty" id="qty" placeholder=""></div><div class="form-group col-md-3"><label class="onboard" for="tax">Tax</label><input type="text" class="form-control dark form-control-sm" id="tax" placeholder=""></div><div class="form-group col-md-3"><label class="onboard" for="discount">Discount</label><input type="text" class="form-control dark form-control-sm line_discount" placeholder=""></div><div class="form-group col-md-3"><label class="onboard" for="discount">Net Total</label><p type="text" class="form-control dark form-control-sm total" id="total" placeholder=""></p></div></div>');
